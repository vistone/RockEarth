...
export class TextureDecoder {
    private static arrayBufferCopy(src: Uint8Array, dst: Uint8Array, dstByteOffset: number, numBytes: number) {
    ...
    public static unpackTextureFormat(available_texture_formats: number | undefined, default_available_texture_formats: number): number {
        let texture_format = 1; // Default to JPG
        const supported = [6, 1]; // [TextureFormat.Texture_Format_CRN_DXT1, TextureFormat.Texture_Format_JPG];
    ...
    public static unpackImageryEpoch(flags: number, imagery_epoch: number | undefined, default_imagery_epoch: number): number {
    ...
    public static unpackUvOffsetAndScale(texture: ITexture, uv_offset_and_scale: number[]) {
        let u_mod = texture.format == 6 ? (texture.width ?? 0) : 0xffff + 1;
        let v_mod = texture.format == 6 ? (texture.height ?? 0) : 0xffff + 1;

        const uv_offset = {x: 0.5, y: 0.5};
        const uv_scale = {x: 1.0 / u_mod, y: 1.0 / v_mod};
    ...
    public static unpackTexCoords(packed: Uint8Array, texture: ITexture, verticesLength: number): Uint16Array {
        if (!(verticesLength * 4 == (packed.length - 4) && packed.length >= 4)) {
            throw new Error("Invalid packed size");
        }
        ...
        for (let i = 0, j = 0; i < verticesLength; i++, j+=2) {
    ...
    public static unpackCRN(data: ArrayBuffer | Uint8Array, width: number, height: number, convertToBMP = true): Uint8Array | Uint16Array {
        ...
        let texture: Uint8Array|Uint16Array = new Uint8Array(crn.Module.HEAPU8.buffer, dst, dstSize);
    ...
    private static dxtToRgb565(src: Uint16Array, src16Offset: number, width: number, height: number): Uint16Array {
